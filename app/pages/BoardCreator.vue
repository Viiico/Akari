    ...
    <main class="container min-h-screen text-white p-4">
        <div class="mb-6 flex flex-col items-center gap-4">
            ...
            <div class="flex flex-wrap justify-center gap-4 mb-4">
                <button v-for="size in boardSizes" :key="size.val" @click="initCreatorBoard(size.val)"
                    class="px-5 py-2 rounded font-medium transition-all duration-200 border" :class="currentSize === size.val ?
            ...
            <div v-if="isCreatorBoardValid && validateWalls()"
                ...
                <p class="text-sm text-green-200 mt-1">All cells are correctly placed and walls have the required bulbs.
                </p>
            </div>
        ...
        <div class="flex justify-center mb-8 relative">
            <div v-if="boardGrid.length > 0"
                class="inline-block p-4 bg-gray-900 border border-gray-700 rounded-lg shadow-xl select-none"
                @contextmenu.prevent>
                    ...
                    <div v-for="(row, rIndex) in boardGrid" :key="rIndex" class="contents">
                        <template v-for="(cell, cIndex) in row" :key="cIndex">
                            <div @click="handleLeftClick(rIndex, cIndex)"
                                @contextmenu.prevent="handleRightClick(rIndex, cIndex)" :class="[
                                ...
                                <div v-if="cell.type === CellType.WALL"
                                    ...
                                    <div v-if="cell.state >= 0 && cell.state <= 4"
                                        class="absolute bottom-1 right-1 w-2 h-2 rounded-full" :class="countAdjacentBulbs(rIndex, cIndex) === cell.state ?
        ...
        <div v-if="isModalOpen" class="fixed inset-0 bg-black/90 flex items-center justify-center z-50 backdrop-blur-sm"
            @click.self="closeModal">
            <div
                class="bg-gray-900 p-8 rounded-xl border border-gray-700 shadow-2xl flex flex-col items-center max-w-md w-full mx-4">
                ...
                <div class="grid grid-cols-2 gap-6 w-full">
                    ...
                    <div @click="selectWallType"
                        ...
                        <div
                            class="w-20 h-20 bg-black border border-gray-600 rounded flex items-center justify-center relative mb-3">
                            <span class="text-gray-300 font-bold text-lg">WALL</span>
                            <span
                                class="absolute top-2 left-2 text-gray-400 text-xs font-mono group-hover:text-blue-400">1</span>
                        </div>
                    ...
                    <div @click="selectBulbType"
                        ...
                        <div
                            class="w-20 h-20 bg-gray-900 border border-gray-600 rounded flex items-center justify-center relative mb-3">
                            <NuxtImg preload format="webp" src="/lightbulb.png" alt="Lightbulb"
                                class="w-10 h-10 brightness-125" />
                            <span
                                class="absolute top-2 left-2 text-gray-400 text-xs font-mono group-hover:text-yellow-400">2</span>
                        </div>
        ...
        <div v-if="isWallInputOpen"
            ...
            @click.self="closeModal">
            <div
                class="bg-gray-900 p-8 rounded-xl border border-gray-700 shadow-2xl flex flex-col items-center max-w-sm w-full mx-4">
                <h3 class="text-xl font-bold text-white mb-2">Wall Value</h3>
                <p class="text-gray-400 text-sm mb-6 text-center">Enter 0-4 for numbered walls, or 5 for solid black
                    wall</p>
        ...
        <div class="flex flex-wrap justify-center gap-4">
            <button @click="printDiv()" class="px-5 py-2 bg-gray-700 hover:bg-gray-600 text-white font-medium rounded">
                Print The Board
            </button>
        </div>
